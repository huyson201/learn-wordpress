...
function _wp_translate_postdata($update = false, $post_data = null)
{
	...
	if (
		isset($post_data['user_ID']) && ($post_data['post_author'] != $post_data['user_ID'])
	...
	if (
		isset($post_data['publish']) && ('' !== $post_data['publish'])
...
function _fix_attachment_links($post)
{
	...
	foreach ($link_matches[1] as $key => $value) {
		if (
			!strpos($value, '?attachment_id=') || !strpos($value, 'wp-att-')
...
function get_sample_permalink_html($id, $new_title = null, $new_slug = null)
{
	...
	if (false === strpos($permalink, '%postname%') && false === strpos($permalink, '%pagename%')) {
		...
		if (
			!get_option('permalink_structure') && current_user_can('manage_options')
...
function post_preview()
{
	...
	if (
		!wp_check_post_lock($post->ID) && get_current_user_id() == $post->post_author
